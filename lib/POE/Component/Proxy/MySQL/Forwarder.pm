...
#   print "[$heap->{log}] Accepted connection from $peer_host:$peer_port in session $session and pid $$ \n";
...
#   print "+ $$ forwarder_client_error \n";
#   print "+ $$ connections_per_child ".$server_heap->{connections_per_child}->{$$}." \n";
#   print "+ $$ active_connections ".$server_heap->{active_connections}->{$$}." \n";
#   print "+ $$ is_a_child ".$server_heap->{is_a_child}." \n";
...
#   print "server_send_query ".$opt->{query}." \n";
   ...
   if ($opt->{callback} ne '') {
#      print "server_send_query client ".$self->client_wheel->ID." will dispatch to callback ".$opt->{callback}." \n";
      $wheel->event(InputEvent => 'dispatch_input');
   }
   else {
#      print "server_send_query will relay as is \n";
...
#         print '$heap->{server_request_callback} = '.$heap->{server_request_callback}."\n";
